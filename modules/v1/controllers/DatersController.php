...
class DatersController extends Controller
{
	...
	{ 
		...
		$query = (new \yii\db\Query ())->select('daters.*,users.phone,users.nickname,users.thumb')->from('daters')->orderBy('created_at desc')->join('INNER JOIN','users','daters.userid = users.id');
