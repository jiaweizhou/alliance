...
class ApplyjobsController extends Controller
{
	...
	{ 
		...
		$query = (new \yii\db\Query ())->select('applyjobs.*,users.phone,users.nickname,users.thumb')->from('applyjobs')->orderBy('create_at desc')->join('INNER JOIN','users','applyjobs.userid = users.id');
